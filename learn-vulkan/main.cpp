...
bool isDeviceSuitable(VkPhysicalDevice device, VkSurfaceKHR surface) {
  ...
  return isComplete(&indices) && allExtensionsSupported && swapChainOkay && features.samplerAnisotropy;
}
...
const std::vector<Vertex> vertices = {{{-0.5, -0.5}, {1.0f, 0.0f, 0.0f}},
...
private:
  ...
  void createImage(uint32_t width, uint32_t height, VkFormat format,
    ...
    allocInfo.allocationSize = imgMemReq.size;
    allocInfo.memoryTypeIndex =
        findMemoryType(physicalDevice, imgMemReq.memoryTypeBits, properties);
  ...
  VkCommandBuffer beginSingleTimeCommands() const {
    ...
    // homework:
    // All of the helper functions that submit commands so far have been set up
  ...
  void transitionImageLayout(VkImage image, VkFormat format,
    ...
    // set src and dst queues to ignored when
    // you're not doing queue family ownership transfer
  ...
  void createUniformBuffers() {
    ...
    VkMemoryRequirements memReq;
    vkGetBufferMemoryRequirements(device, uniformBuffers[0], &memReq);

    VkMemoryAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    allocInfo.allocationSize = memReq.size * uniformBuffers.size();
    allocInfo.memoryTypeIndex =
        findMemoryType(physicalDevice, memReq.memoryTypeBits, propertyFlags);
    if (vkAllocateMemory(device, &allocInfo, nullptr, &uniformBuffersMemory) !=
        VK_SUCCESS) {
      throw std::runtime_error("failed to allocate vertex buffer memory!");
    }

    for (size_t i = 0; i < uniformBuffers.size(); i++) {
      vkBindBufferMemory(device, uniformBuffers[i], uniformBuffersMemory,
                         memReq.size * i);
    }
  }
  ...
  void updateUniformBuffer(uint32_t imageIndex) {
    ...
    ubo.view =
        glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.f, 0.f, 0.f),
                    glm::vec3(0.0f, 0.0f, 1.0f));
    ...
    void *data;
    vkMapMemory(device, uniformBuffersMemory,
                imageIndex * 256, // todo, 256 comes from the memReq of the UBO
